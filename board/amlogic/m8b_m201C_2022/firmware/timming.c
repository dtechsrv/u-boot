...
//Following setting for board XXXXXXX with DDR K4B4G1646B(SANSUNG)
...
#ifdef LPDDR2
	//DTPR0
	#define CFG_DDR_RTP  (4)  //pctl need use 0
	#define CFG_DDR_WTR  (4)
	#define CFG_DDR_RP   (12)
	#define CFG_DDR_RPAB (12)
	#define CFG_DDR_RPPB (10)
	...
	//DTPR1
	#define CFG_DDR_MRD   (5)
	#define CFG_DDR_MOD   (0)
	#define CFG_DDR_FAW   (27)
	#define CFG_DDR_RFC   (70) //4Gb 1066
	#define CFG_DDR_WLMRD (0)
	#define CFG_DDR_WLO   (0)
	...
	#define CFG_DDR_CL    (8) //RL
	#define CFG_DDR_WR    (8)
	#define CFG_DDR_CWL   (4) //WL
	#define CFG_DDR_AL    (0)
	#define CFG_DDR_EXSR  (76) //For LPDDR2/LPDDR3, this should be programmed to match tXSR.  tRFCab + 10ns
	#define CFG_DDR_DQS   (4) // for g2 phy (2)
	#define CFG_DDR_CKSRE (0)
	#define CFG_DDR_CKSRX (2)
	#define CFG_DDR_ZQCS  (48)
	#define CFG_DDR_ZQCL  (192)
	#define CFG_DDR_XPDLL (0)  // pctl
	#define CFG_DDR_ZQCSI (1000)
	#define CFG_DDR_TDQSCK    (1)
	#define CFG_DDR_TDQSCKMAX (1)
	#define CFG_DDR_TCKESR    (8)
	#define CFG_DDR_TDPD      (500)
	...
	#define CFG_DDR_RTP  (6)
	#define CFG_DDR_WTR  (6)
	#define CFG_DDR_RP   (17)
	#define CFG_DDR_RPAB (17)
	#define CFG_DDR_RPPB (15)
	#define CFG_DDR_RCD  (15)
	#define CFG_DDR_RAS  (34)
	#define CFG_DDR_RRD  (8)
	#define CFG_DDR_RC   (51)

	//DTPR1
	#define CFG_DDR_MRD   (11)  //MAX is 11 in PUB(8~11).PCTL range is 0~7
	#define CFG_DDR_MOD   (0)
	#define CFG_DDR_FAW   (40)
	#define CFG_DDR_RFC   (104) //4Gb 1600
	#define CFG_DDR_WLMRD (32)
	#define CFG_DDR_WLO   (8)    //MAX 20ns
	...
	#define CFG_DDR_REFI  (39)
	#define CFG_DDR_REFI_MDDR3  (0)
		
	#define CFG_DDR_CL    (12)   //RL
	#define CFG_DDR_WR    (12)
	#define CFG_DDR_CWL   (6)    //WL
	#define CFG_DDR_AL    (0)
	#define CFG_DDR_EXSR  (112)
	#define CFG_DDR_DQS   (4)
	#define CFG_DDR_CKSRE (2)
	#define CFG_DDR_CKSRX (2)
	#define CFG_DDR_ZQCS  (72)
	#define CFG_DDR_ZQCL  (288)
	#define CFG_DDR_XPDLL (0)   //pctl
	#define CFG_DDR_ZQCSI (1000)
	#define CFG_DDR_TDQSCK    (3)
	#define CFG_DDR_TDQSCKMAX (5)
    #define CFG_DDR_TCKESR    (12)
	#define CFG_DDR_TDPD      (500)
#endif
...
static struct ddr_set __ddr_setting={
	...
	.t_ddr_apd_ctrl = ( 32 << 0 ) | //[B7..B0]: DMC active latency.  latency to enter LOW POWER state after the the DMC is not actived.
                      ...
                      ( 1  << 19) | //bit 19.     1 :  LPDDR2/3 share pins mode. ras_n => cke_1.  cas_n => cs1_n.   we_n => odt_1.  0:  normal mode,  use physical cs1_n, cke_1 and odt_1 I/O pins.
    #endif
					  ( 1  << 16), 	//[B17,B16]: DDR mode to genreated the refresh ack signal and prcharge signal to DMC for dmc refresh control.
	...
	.t_pctl_mcfg =   1 |	//[B0] mem-bl: 0 for 4; 1 for 8
...
#if (defined LPDDR2) || (defined LPDDR3)              
              (1 << 6) |    //Enables LPDDR2-S4 support.1'b0 = LPDDR2-S4 disabled (LPDDR2-S2 enabled)1'b1 = LPDDR2-S4 enabled 
              (1 << 7) |
              (2 << 20)|    //mDDR/LPDDR2 Burst Length. The BL setting must be consistent with the value programmed into the BL field of MR. 
              ...
              (0x2f << 24) |    //Clock stop idle period in n_clk cycles. Clock stop mode is only applicable in mDDR/LPDDR2/LPDDR3.
#endif
#ifdef LPDDR2
              (3 << 22)|    //mDDR/LPDDR2 Enable. Enables support for mDDR or LPDDR2 or LPDDR3.
    ...
    .t_pctl_tmrd   = (CFG_DDR_MRD>7)?7:CFG_DDR_MRD,
#if (defined LPDDR2) || (defined LPDDR3)
	.t_pctl_trp    = (unsigned short)(((CFG_DDR_RPAB-CFG_DDR_RPPB)<<16)|CFG_DDR_RPPB),
#else
    ...
    .t_pub_mr={
		[0]=0,       						      
       	[1]=(3 << 0)|(1 << 3)|(0 << 4)|((CFG_DDR_WR - 2) << 5),//OP<2:0> 010B:BL4 (default) 011B:BL8 100B:BL16. OP<3>0B:Sequential 1B:Interleaved.  OP<4>0B:Wrap 1B:No wrap.
        [2]=(CFG_DDR_CL - 2),
        [3]=1,//OP<3:0> 0001B:34.3-ohm typical 0010B:40-ohm typical (default) 0011B:48-ohm typical 0100B: 60-ohm typical 0101B:reserved for 68.6-ohm typical 0110B: 80-ohm typical 0111B: 120-ohm typical (optional)
    },
#elif defined LPDDR3
    .t_pub_mr={
		[0]=0,    				      
       	[1]=(3 << 0)|(((CFG_DDR_WR > 9)?(CFG_DDR_WR - 10):(CFG_DDR_WR - 2)) << 5),//OP<2:0>011B: BL8 (default).  OP<7:5>If nWRE (MR2 OP<4>) = 0:001B: nWR=3 (optional) 100B: nWR=6 110B: nWR=8 111B: nWR=9. If nWRE (MR2 OP<4>) = 1:000B: nWR=10 (default) 001B: nWR=11 010B: nWR=12 100B: nWR=14 110B: nWR=16
	   	[2]=(CFG_DDR_CL - 2)|  //OP<3:0>
                                // If OP<6> =0 (WL Set A, default)
                                // 0001B: RL = 3 / WL = 1 (≤166 MHz, optional)
                                ...
                                // 1110B: RL = 16 / WL = 13 (≤1066 MHz)
            (((CFG_DDR_WR > 9)?1:0) << 4)|//OP<4> 0B:enable nWR programming ≤9 1B:enable nWR programming > 9
#ifdef CONFIG_ENABLE_WRITE_LEVELING
            (1 << 7),//WR Leveling. OP<7> 0B:disabled (default) 1B:enabled
#else
            (0 << 7),//WR Leveling. OP<7> 0B:disabled (default) 1B:enabled
#endif
	   	[3]=3,  //OP<3:0>
                ...
                // 1001B:34.3Ωtypical pull-down, 40Ωtypical pull-up
                // 1010B:40Ωtypical pull-down, 48Ωtypical pull-up
                // 1011B:34.3Ωtypical pull-down, 48Ωtypical pull-up
	},
#else
	.t_pub_mr={
	...
	.t_pub_dtpr={
...
#ifdef LPDDR3
                ((CFG_DDR_RPAB - 8) << 8  )|       //tRP        //12500ps
                ((CFG_DDR_RCD - 8)  << 12 )|       //tRCD       //12500ps
#else
		...
		[1] =
#ifdef LPDDR3
                (((CFG_DDR_MRD - 8) << 0 ) |		//tMRD      //4 TCK
#else
                (((CFG_DDR_MRD - 4) << 0 ) |		//tMRD      //4 TCK
#endif
#if (!defined LPDDR2) && (!defined LPDDR3)
